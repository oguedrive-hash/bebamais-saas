...
async function tratarQuedaNumero(instanceCaido: string): Promise<void> {
  ...
  for (const l of leads) {
    ...
    const override = `Voce e ${nomeNovo}, atendente da empresa. Voce esta ASSUMINDO AGORA este atendimento, que estava com ${nomeAntigo} (a conexao do WhatsApp dele caiu). O historico da conversa esta acima. Envie UMA mensagem curta e natural: cumprimente a pessoa pelo nome, avise de forma leve que vai continuar o atendimento no lugar do ${nomeAntigo} e, SE — e somente se — o historico tiver um ponto CONCRETO que a pessoa falou (item do pedido, retirada ou entrega, endereco), retome esse ponto; se nao tiver nada concreto, so pergunte como pode ajudar. NUNCA invente dados e NUNCA repita exemplos desta instrucao como se fossem fatos. 1-2 frases, no maximo uma pergunta. Tom natural, sem parecer robo. PROIBIDO emoji.`;
